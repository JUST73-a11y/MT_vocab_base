    ...
    return (
        <>
                    ...
                    <div className="flex items-center justify-between h-20">
                        <div className="flex items-center gap-10">
                            <Link href="/admin/dashboard" className="flex items-center gap-2 group">
                                ...
                                <span className="font-black text-xl tracking-tighter text-white">AdminPanel</span>
                            </Link>
            ...
            <nav className="md:hidden mobile-bottom-nav">
                {navItems.map(item => {
                    const currentTab = searchParams.get('tab') || '';
                    const active = item.name === 'Overview' ? !currentTab : currentTab === item.name.toLowerCase();
                    return (
                        <Link key={item.href} href={item.href}
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    return (
        <div className="flex flex-col items-center min-h-screen w-full bg-[#0a0a0f] font-sans text-white relative overflow-hidden">
            <MeshBackground />
            <Suspense fallback={<div className="min-h-screen bg-[#0a0a0f]" />}>
                <AdminNavContent />
                <main className="w-[95%] lg:w-[80%] max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full relative z-10">
                    {children}
                </main>
            </Suspense>
        </div>
