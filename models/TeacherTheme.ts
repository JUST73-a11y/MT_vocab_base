...
const TeacherThemeSchema = new Schema({
  teacherId:  { type: Schema.Types.ObjectId, ref: 'User', required: true, index: true },
  name:       { type: String, required: true, trim: true, maxlength: 50 },
  config:     { type: ThemeConfigSchema, default: () => ({}) },
  isEquipped: { type: Boolean, default: false, index: true },
  source:     { type: String, enum: ['SYSTEM','TEACHER'], default: 'TEACHER' },
}, { timestamps: true });

TeacherThemeSchema.index({ teacherId: 1, isEquipped: 1 });

if (process.env.NODE_ENV !== 'production' && mongoose.models.TeacherTheme) {
  delete (mongoose.models as any).TeacherTheme;
}

const TeacherTheme = mongoose.models.TeacherTheme || mongoose.model('TeacherTheme', TeacherThemeSchema);
